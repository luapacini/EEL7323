#include "SHT30.h"
#include "Serial_0.h"
#include "I2C.h"
#include <util/delay.h>
#include <stdio.h>

uint8_t leitura[6];

uint16_t rawTemperature, rawHumidity;
uint8_t tempCRC, humCRC;
uint16_t temp, umidade;
uint8_t breakFlag = 0;

uint8_t initCRC = 0xFF; //de acordo com datasheet
uint8_t CRC_lookup[256] =  {};



uint8_t crc8(uint8_t *msg, int lengthOfMsg, uint8_t init) {
  while (lengthOfMsg-- > 0) init = CRC_lookup[ (init ^ *msg++)];
  return init;
}

void periodicAquisitionSetup() {
    uint8_t msg[2];

    msg[0] = 0x21; //tabela 10 datasheet, 1mps, MSB
    msg[1] = 0x30;//tabela 10 datasheet, high repeatability, LSB

    I2C_WriteData(SHT30_ADDRESS, msg, 2); // Writes the data
    Serial_0_println("--- Periodic Aquisition Mode ---"); Serial_0_println(" ");

    _delay_ms(3000);
}

uint8_t periodicAquisition() {
    uint8_t msg[2];

    msg[0] = 0xE0; //tabela 11 datasheet /fetch data/
    msg[1] = 0x00; //tabela 11 datasheet /fetch data/

    I2C_WriteData(SHT30_ADDRESS, msg, 2); // Writes the data
    
    I2C_ReadData(SHT30_ADDRESS, leitura, 6);

    if (breakFlag == 2) return 0; //nao imprimir duas vezes "Break conluido"

    if (breakFlag == 1) {
        Serial_0_println("Break concluido");
        breakFlag++;
        return 0;
    }

    uint16_t rawTemperature = (uint16_t)((((uint16_t)leitura[0]) << 8) | leitura[1]); // 8MSB da temperatura deslocados pra esquerda e ou logico com 8LSB
    uint8_t tempCRC = leitura[2];
    uint16_t rawHumidity = (uint16_t)((((uint16_t)leitura[3]) << 8) | leitura[4]); //mesma coisa umidade
    uint8_t humCRC = leitura[5];

    if (crc8(&(leitura[0]), 2, initCRC) == tempCRC && crc8(&(leitura[3]), 2, initCRC) == humCRC) {
        // Calculate temperature and humidity  
        temp = -45000 + 175000 * (rawTemperature / 65535.0); 
        umidade = 100 * (rawHumidity / 65535.0);

        char buffer[10];
        snprintf(buffer, 10, "%u", (unsigned int)(temp));
        Serial_0_print("Temperatura = "); Serial_0_print(buffer); Serial_0_println(" mºC");
        snprintf(buffer, 10, "%u", (unsigned int) umidade);
        Serial_0_print("Umidade = "); Serial_0_print(buffer); Serial_0_println("%"); Serial_0_println("");
        _delay_ms(2000);
    } else {
        Serial_0_println("Houve problema no CRC, nao eh possivel printar resultado");
    }
    return 0;
}

uint8_t periodicAquisitionBreak() {
    uint8_t msg[2];

    msg[0] = 0x30; 
    msg[1] = 0x93; 

    I2C_WriteData(SHT30_ADDRESS, msg, 2); // Writes the data

    Serial_0_println("--- Break ---"); Serial_0_println(" ");
    return 1;
}

void oneShotAquisitionSetup() {
    uint8_t msg[2];

    msg[0] = 0x2C; //tabela 9 datasheet, MSB
    msg[1] = 0x06; //tabela 9 datasheet, LSB

    I2C_WriteData(SHT30_ADDRESS, msg, 2); // Writes the data

    Serial_0_println("--- One Shot Aquisition Mode ---"); Serial_0_println("");

    _delay_ms(3000);
}

void oneShotAquisition() {
    I2C_ReadData(SHT30_ADDRESS, leitura, 6);

    uint16_t rawTemperature = (uint16_t)((((uint16_t)leitura[0]) << 8) | leitura[1]); // 8MSB da temperatura deslocados pra esquerda e ou logico com 8LSB
    uint8_t tempCRC = leitura[2];
    uint16_t rawHumidity = (uint16_t)((((uint16_t)leitura[3]) << 8) | leitura[4]); //mesma coisa umidade
    uint8_t humCRC = leitura[5];

    if (crc8(&(leitura[0]), 2, initCRC) == tempCRC && crc8(&(leitura[3]), 2, initCRC) == humCRC) {
        // Calculate temperature and humidity  
        temp = -45000 + 175000 * (rawTemperature / 65535.0); 
        umidade = 100 * (rawHumidity / 65535.0);

        char buffer[10];
        snprintf(buffer, 10, "%u", (unsigned int)(temp));
        Serial_0_print("Temperatura = "); Serial_0_print(buffer); Serial_0_println(" mºC");
        snprintf(buffer, 10, "%u", (unsigned int) umidade);

        Serial_0_print("Umidade = "); Serial_0_print(buffer); Serial_0_println("%"); Serial_0_println("");
        _delay_ms(2000);
    } else {
        Serial_0_println("Houve problema no CRC, nao eh possivel printar resultado");
    }
}
